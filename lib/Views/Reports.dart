      ...
      providers: [
        BlocProvider(
          create: (context) => MonthlyReadingsBloc(
              RepositoryProvider.of<MonthlyReadingService>(context),
              RepositoryProvider.of<generalService>(context))
            ..add(gettypes())
  ...
  _page(BuildContext context) {
    return SingleChildScrollView(
      child: Column(
        children: [
          ScreenTypeLayout(
            mobile: ReportHeaderDesktop(getyear, setyear),
            desktop: ReportHeaderDesktop(getyear, setyear),
            tablet: ReportHeaderDesktop(getyear, setyear),
          ),
...
class MonthlyGraphsReports extends StatelessWidget {
          ...
          return Container(
              ...
              child: ScreenTypeLayout(
                mobile: ListViewBuilder(
                  temp,
                  type: 'Monthly Readings Report',
                  optional1: year,
                ),
                tablet: GridViewBuilder(temp, 1,
                    type: 'Monthly Readings Report', optional1: year),
                desktop: GridViewBuilder(temp, 2,
                    type: 'Monthly Readings Report', optional1: year),
              ));
        }));
  }
  ...
  List<List<MonthlyModel>> gettypesfunc(BuildContext context) {
    ...
    List<List<MonthlyModel>> output = List.generate(
        typesunique.length,
        (index) => temp
